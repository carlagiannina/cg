						...
						if( ! post_password_required() ) {
							...
							*/
							if( empty( $post_format ) ) {
								...
								/* featured image */
								$hidden_portfolio .= '<img class="ut-portfolio-image ut-load-me ' . $image_style . '" alt="' . get_the_title() . '" data-original="' . $fullsize . '">';
							...
							*/
							if( $post_format == 'video' ) {
								...
								$hidden_portfolio .= '<div id="ut-video-call-'.$portfolio_query->post->ID.'" class="ut-video-call" data-video-url="' . $video_url . '"></div>';
							...
							$title= str_ireplace('"', '', trim(get_the_title()));
								...
								if( $detailstyle == 'popup' ) {
									...
									if( $post_format == 'video' ) {
										...
										/* try to catch video url */
										$popuplink = ut_get_portfolio_format_video_content( $content );
			...
			if ($portfolio_query->have_posts()) : while ($portfolio_query->have_posts()) : $portfolio_query->the_post();
				...
				$masonry .= '<div class="ut-masonry ut-hover">';
						...
						if ( has_post_thumbnail() && ! post_password_required() ) :		
							...
							$masonry .= '<figure>';
								$masonry .= '<img alt="' . get_the_title() . '" width="'.$width.'" height="'.$height.'" src="' . UT_PORTFOLIO_URL . 'i/?w='.$width.'&h='.$height.self::$cache.'" class="portfolio-lazy" alt="' . $title . '" data-original="' . $thumbnail . '" />';
							$masonry .= '</figure>';
						...
						$gallery .= '<div class="ut-portfolio-item ut-hover">';
							...
							if( $detailstyle == 'popup' ) {
								...
								if( $post_format == 'gallery' ) {
									...
									$gallery .= '<a class="ut-portfolio-popup-'.$portfolio_query->post->ID.' ' . $portfolio_settings['image_style'] . '" title="' . $caption . '" href="'.$fullsize.'">';									
							...
							if ( has_post_thumbnail() && ! post_password_required() ) :		
								...
								$gallery .= '<figure><img alt="' . get_the_title() . '" width="'.$width.'" height="'.$height.'" src="' . UT_PORTFOLIO_URL . 'i/?w='.$width.'&h='.$height.self::$cache.'" class="portfolio-lazy" alt="' . $title . '" data-original="' . $thumbnail . '" /></figure>';
